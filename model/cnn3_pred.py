...
class YieldPredictionModel(nn.Module):
    ...
    def forward(self, x, y):
        ...
        # 提取时空特征（输出形状: (B,256,T,1,1)）
        spatial_feat = self.backbone(x).squeeze(-1).squeeze(-1)  # (B,256,T)

        # 时序建模（调整为Transformer需要的形状）
        temporal_feat = rearrange(spatial_feat, 'b d t -> t b d')  # (T,B,256)
        temporal_feat = self.temporal_encoder(temporal_feat)
        temporal_feat = temporal_feat.mean(dim=0)  # (B,256)
...
if __name__ == "__main__":
    ...
    dummy_input = torch.randn(2, 32, 4, 128, 128)
    y = torch.randn(2, 32, 4, 128, 128)
    model = YieldPredictionModel(time_steps=12, in_channels=4+4)
    output = model(dummy_input, y)

    print("输入形状:", dummy_input.shape)  # torch.Size([2, 12, 3, 128, 128])
