...
class Kansformer(nn.Module):
    def __init__(self, x_channels, y_channels):
        ...
        # d_model = 512
        d_model = 256
        if opt.upscaling_layer:
            # self.feature_upscaling_layer_x = FeatureUpscalingLayer(d_model, d_model * 2)
            # self.feature_upscaling_layer_y = FeatureUpscalingLayer(d_model, d_model * 2)

            self.conv1x1_x = nn.Conv2d(d_model, d_model * 2, kernel_size=1)
            self.rl_x = nn.LeakyReLU()

            self.conv1x1_y = nn.Conv2d(d_model, d_model * 2, kernel_size=1)
            self.rl_y = nn.LeakyReLU()
        if opt.upscaling_layer:
            d_model = d_model * 2
