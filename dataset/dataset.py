...
class ImageSequenceDataset(Dataset):
    def __init__(self, folders_09a1, folders_11a2, hdf5_09a1='data/data_09a1.h5', hdf5_11a2='data/data_11a2.h5',
        ...
        if not self.check_file_exists(hdf5_09a1):
            self._build_or_load_h5(hdf5_09a1, folders_09a1, 7)
        if not self.check_file_exists(hdf5_11a2):
            self._build_or_load_h5(hdf5_11a2, folders_11a2, 2)
        print(f'load to memory===============')
        self.data_09a1 = self._load_data_to_memory(folders_09a1, 7, hdf5_09a1)
        self.data_11a2 = self._load_data_to_memory(folders_11a2, 2, hdf5_11a2)
    ...
    def _build_or_load_h5(self, hdf5_file, folders, channel):
        # 检查h5文件是否存在
        if os.path.exists(hdf5_file):
            data = h5py.File(hdf5_file, 'r')
        else:
            print(f'No H5 file {hdf5_file}, create h5 file===============================')
            data = self._build_h5(hdf5_file, folders, channel)
        return data

    def _build_h5(self, hdf5_file, folders, channel):
        with h5py.File(hdf5_file, 'w') as data:
            for i, folder in enumerate(folders):
                ...
                images = data.create_dataset(f'dataset_{i}', (length, channel, opt.img_shape, opt.img_shape),
                                             dtype='float32')
    ...
    def _load_data_to_memory(self, folders, channel, hdf5_file):
        if os.path.exists(hdf5_file):  # 判断 h5 文件是否存在
            ...
            for i, folder in enumerate(folders):
                ...
                files = sorted(files,
                               key=lambda x: datetime.strptime(os.path.splitext(os.path.splitext(x)[0])[0], '%Y-%m-%d'))
                images = np.empty((length, channel, opt.img_shape, opt.img_shape), dtype=np.float32)
                for j, file in enumerate(files):
                    with rasterio.open(os.path.join(folder, file)) as src:
                        img = src.read()
                    img = self._preprocess_image(img)
    ...
    def _preprocess_image(self, img):
        ...
        """
        if img.shape[1] > opt.img_shape or img.shape[2] > opt.img_shape:
            raise ValueError(f'image shape error! {img.shape[1]},{img.shape[2]}')
        if img.shape[1] < opt.img_shape:
            # 如果图像宽度小于目标宽度，则在右侧进行零填充
            padding = np.zeros((img.shape[0], opt.img_shape - img.shape[1], img.shape[2]), dtype=img.dtype)
            img = np.concatenate((img, padding), axis=1)
        if img.shape[2] < opt.img_shape:
            # 如果图像高度小于目标高度，则在底部进行零填充
            padding = np.zeros((img.shape[0], img.shape[1], opt.img_shape - img.shape[2]), dtype=img.dtype)
    ...
    def __getitem__(self, idx):
        ...
        return data_09a1, data_11a2, self.labels[idx], self.path[idx], self.area[idx]
