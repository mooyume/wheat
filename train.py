...
def train():
    ...
    for epoch in range(opt.n_epochs):
        ...
        for i, (mod09a1, fldas, labels, path, history_data) in enumerate(train_dataloader):
            mod09a1 = mod09a1.to(device)
            fldas = fldas.to(device)
            history_data = history_data.to(device)
            labels = labels.view(-1, 1).to(device)
            outputs = model(mod09a1, fldas, history_data)
        ...
        with torch.no_grad():  # 在验证过程中不需要计算梯度
            for x, fldas, labels, path, history_data in val_dataloader:  # 假设val_dataloader是你的验证数据加载器
                x = x.to(device)
                fldas = fldas.to(device)
                history_data = history_data.to(device)
                labels = labels.view(-1, 1).to(device)
        ...
        metrics_map = calculate_metrics(pred_metric, real_metric)
        mape, rmse, mae, r2 = metrics_map['MAPE'], metrics_map['RMSE'], metrics_map['MAE'], metrics_map['R2']
        # text = f'remove:{k}, Epoch [{epoch + 1}/{opt.n_epochs}], Validation Loss: {val_loss}, MAPE: {mape}, RMSE: {rmse}, MAE: {mae}, R2: {r2}'
        text = f'Epoch [{epoch + 1}/{opt.n_epochs}], Validation Loss: {val_loss}, MAPE: {mape}, RMSE: {rmse}, MAE: {mae}, R2: {r2}'
        logger.info(text)
